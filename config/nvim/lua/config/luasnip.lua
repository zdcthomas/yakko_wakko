...
-- some shorthands...
local snippet = luasnip.snippet local snippet_node = luasnip.snippet_node local text_node = luasnip.text_node
...
function elixir.typedstruct()
  return snippet(
    {trig = "typedstruct"},
    {
      text_node("typedstruct "), insert_node(1),
      text_node("do", "field"), insert_node(2),
      text_node("end")
    }
  )
end
...
function conf.setup()
  ...
  vim.api.nvim_set_keymap('i', '<C-J>', 'luasnip#choice_active() ? "<Plug>luasnip-next-choice" : "<C-J>"', {expr = true, silent = true} )
  vim.api.nvim_set_keymap('i', '<C-K>', 'luasnip#choice_active() ? "<Plug>luasnip-prev-choice" : "<C-K>"', {expr = true, silent = true} )
