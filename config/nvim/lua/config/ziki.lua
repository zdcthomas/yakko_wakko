...
---- DEFAULT CONFIG
---- ----------------------------------------------------------------------------
--local home = vim.fn.expand("~/irulan")
--local Module = {}
...
--			"ziki I guess: " .. purpose .. " folder " .. dir .. " does not exist!" .. " Shall I create it? [y/N] "
--		)
...
--	local ret
--	if Module.Config.home == nil then
--		print_error("ziki: home is not configured!")
--		ret = false
--	else
--		ret = check_dir_and_ask(Module.Config.home, "home")
--	end
...
--		if vim.fn.getbufvar(i, "&filetype") == "telekasten" and vim.fn.getbufvar(i, "&mod") == 1 then
--			vim.cmd(i .. "bufdo w")
--		end
--	end
--end
...
--	dates.rfc3339 = os.date(df.date, time) .. os.date("T%H:%M:%S") .. "Z" .. zonehour .. ":" .. zonemin
...
--	if Module.Config.calendar_opts.calendar_monday == 1 then
--		sunday_offset = 7
--	end
...
----- resolve_path(filepath, opts)
----- inspects the path and returns a Pinfo table
--function Pinfo:resolve_path(filepath, opts)
--	opts = opts or {}
--	-- opts.subdirs_in_links = opts.subdirs_in_links or Module.Config.subdirs_in_links

--	self.fexists = file_exists(filepath)
--	self.filepath = filepath
--	self.root_dir = Module.Config.home
...
--	self.filename = split_path[#split_path]
--	self.title = self.filename:gsub(Module.Config.extension, "")

--	-- if vim.startswith(p, Module.Config.home) then
--	self.root_dir = Module.Config.home
--	-- end
--	-- if vim.startswith(p, Module.Config.dailies) then
--	-- 	self.root_dir = Module.Config.dailies
...
--	-- 	self.is_daily = true
--	-- end
--	-- if vim.startswith(p, Module.Config.weeklies) then
--	-- 	-- TODO: parse "title" into calendarinfo like in resolve_link
--	-- 	-- not really necessary as the file exists anyway and therefore we don't need to instantiate a template
--	-- 	self.root_dir = Module.Config.weeklies
--	-- 	self.is_daily_or_weekly = true
--	-- 	self.is_weekly = true
--	-- end

--	-- now work out subdir relative to root
--	self.sub_dir = filepath
--		:gsub(escape(self.root_dir .. "/"), "")
--		:gsub(escape(self.filename), "")
--		:gsub("/$", "")
--		:gsub("^/", "")

--	if #self.sub_dir > 0 then
...
--	opts = opts or {}
--	opts.weeklies = opts.weeklies or Module.Config.weeklies
--	opts.dailies = opts.dailies or Module.Config.dailies
--	opts.home = opts.home or Module.Config.home
--	opts.extension = opts.extension or Module.Config.extension
--	opts.template_handling = opts.template_handling or Module.Config.template_handling
--	opts.new_note_location = opts.new_note_location or Module.Config.new_note_location
...
--			self.is_daily, self.is_weekly, self.calendar_info = check_if_daily_or_weekly(self.title)
...
--	self.template = Module.note_type_templates.normal
...
--			self.template = Module.note_type_templates.daily
--		elseif self.is_weekly then
--			self.template = Module.note_type_templates.weekly
--		else
--			self.template = Module.note_type_templates.normal
--		end
--	end
...
--	local new_fl = {}
--	ftypes = ftypes or { Module.Config.extension }
...
--Module.base_directory = vim.fn.fnamemodify(sourced_file, ":h:h:h")
--local media_files_base_directory = Module.base_directory .. "/telescope-media-files.nvim"
...
--	local filter_extensions = opts.filter_extensions or Module.Config.filter_extensions
--	file_list = filter_filetypes(file_list, filter_extensions)
--	table.sort(file_list, order_numeric)

--	local counts = nil
--	if opts.show_link_counts then
--		counts = linkutils.generate_backlink_map(Module.Config)
--	end
...
--	if Module.Config.auto_set_filetype then
...
--		fn = fn:gsub(escape(Module.Config.home .. "/"), "")
--		local imglink = "![](" .. fn .. ")"
...
--	opts = opts or {}
--	opts.insert_after_inserting = opts.insert_after_inserting or Module.Config.insert_after_inserting
--	opts.close_after_yanking = opts.close_after_yanking or Module.Config.close_after_yanking
...
--	local fname = Module.Config.dailies .. "/" .. today .. Module.Config.extension
--	local fexists = file_exists(fname)
--	if
--		(fexists ~= true)
--		and ((opts.dailies_create_nonexisting == true) or Module.Config.dailies_create_nonexisting == true)
--	then
--		create_note_from_template(today, fname, Module.note_type_templates.daily)
--		opts.erase = true
...
--		cwd = Module.Config.dailies,
--		find_command = Module.Config.find_command,
...
--	local fname = Module.Config.weeklies .. "/" .. title .. Module.Config.extension
--	local fexists = file_exists(fname)
--	if
--		(fexists ~= true)
--		and ((opts.weeklies_create_nonexisting == true) or Module.Config.weeklies_create_nonexisting == true)
--	then
--		create_note_from_template(title, fname, Module.note_type_templates.weekly)
--		opts.erase = true
--		opts.erase_file = fname
--	end
...
----
--local function selection_to_content(selection)
--	local path = Path:new(selection.path)
--	local split_path = path:_split()
--	local filename = split_path[#split_path]
--	local title = filename:gsub(Module.Config.extension, "")
--	local sub_dir = selection.path
--		:gsub(escape(Module.Config.home .. "/"), "")
--		:gsub(escape(filename), "")
--		:gsub("/$", "")
--		:gsub("^/", "")

--	if #sub_dir > 0 then
--		return sub_dir .. "/" .. title
--	else
--		return title
--	end
--end

--local function to_link(string)
--	if Module.Config.link_style == "md" then
--		return "(" .. string .. ")[" .. string .. "]"
--	elseif Module.Config.link_style == "wiki" then
--		return "[[" .. string .. "]]"
--	end
--end
...
--	-- I don't know if I want this for now I'll keep it but it's not terribly helpful, and feels a bit like bloat
--	-- if not global_dir_check() then
--	-- 	return
--	-- end
...
--				local selection = action_state.get_selected_entry()
--				local content
--				-- If the user selected something, paste that, otherwise, use whatever they typed
--				if selection then
--					content = selection_to_content(selection)
--				else
--					content = action_state.get_current_line()
--				end

--				print("content")
--				Pr(content)
--				vim.api.nvim_put({ to_link(content) }, "", true, true)
...
--	return taglinks.is_tag_or_link_at(line, col, Module.Config)
--end
...
--		return 'call jobstart(["' .. cmd .. '", "' .. url .. '"], {"detach": v:true})'
--	end
...
--	local fexists = file_exists(Module.Config.home .. "/" .. fname)
...
--		})
--	else
--		print("File not found: " .. Module.Config.home .. "/" .. fname)
--	end
--end
...
--	local title = "[[" .. Pinfo:new({ filepath = vim.fn.expand("%:p"), Module.Config }).title .. "]]"
--	vim.fn.setreg('"', title)
--	print("yanked " .. title)
--end
...
--	local oldfile = Pinfo:new({ filepath = vim.fn.expand("%:p"), Module.Config })

--	local newname = vim.fn.input("New name: ")
--	newname = newname:gsub("[" .. Module.Config.extension .. "]+$", "")
--	local newpath = newname:match("(.*/)") or ""
--	newpath = Module.Config.home .. "/" .. newpath

--	-- If no subdir specified, place the new note in the same place as old note
--	if Module.Config.subdirs_in_links == true and newpath == Module.Config.home .. "/" and oldfile.sub_dir ~= "" then
--		newname = oldfile.sub_dir .. "/" .. newname
--	end

--	local fname = Module.Config.home .. "/" .. newname .. Module.Config.extension
--	local fexists = file_exists(fname)
--	if fexists then
...
--		vim.cmd("saveas " .. Module.Config.home .. "/" .. newname .. Module.Config.extension)
--		vim.cmd("bdelete " .. oldfile.title .. Module.Config.extension)
--		os.execute("rm " .. Module.Config.home .. "/" .. oldfile.title .. Module.Config.extension)
--	end

--	if Module.Config.rename_update_links == true then
...
--		if #(vim.fn.getbufinfo({ bufmodified = 1 })) > 1 and Module.Config.auto_set_filetype == true then
--			local answer = vim.fn.input(
--				"Telekasten.nvim:" .. "Save all telekasten buffers before updating links? [Y/n]"
--			)
--			answer = vim.fn.trim(answer)
...
--		recursive_substitution(Module.Config.home, oldlink, newlink)
--		recursive_substitution(Module.Config.dailies, oldlink, newlink)
--		recursive_substitution(Module.Config.weeklies, oldlink, newlink)
--	end
--end
...
--	local fname = Module.Config.dailies .. "/" .. word .. Module.Config.extension
--	local fexists = file_exists(fname)
--	if
--		(fexists ~= true)
--		and ((opts.follow_creates_nonexisting == true) or Module.Config.follow_creates_nonexisting == true)
--	then
--		create_note_from_template(word, fname, Module.note_type_templates.daily, opts.dates)
--		opts.erase = true
--		opts.erase_file = fname
--	end
...
--		cwd = Module.Config.home,
--		search_dirs = { Module.Config.home },
...
--				create_note_from_template(title, fname, template, pinfo.calendar_info)
--				-- open the new note
--				vim.cmd("e " .. fname)
--				picker_actions.post_open()
--			end)
...
--	title = title:gsub("[" .. Module.Config.extension .. "]+$", "")
--	if #title > 0 then
...
--		create_note_from_template(title, fname, pinfo.template, pinfo.calendar_info)
--		opts.erase = true
--		opts.erase_file = fname
--	end
...
--	if Module.Config.template_handling == "always_ask" then
...
--	if kind == "tag" then
--		if atcol ~= nil then
--			tag = taglinks.get_tag_at(vim.api.nvim_get_current_line(), atcol, Module.Config)
--		end
--		search_mode = "tag"
--		title = tag
--	else
...
--				create_note_from_template(title, pinfo.filepath, pinfo.template, pinfo.calendar_info)
--				opts.erase = true
...
--			display, hl_group = utils.transform_devicons(display_entry.filn, display, false)
...
--				return function(t)
--					local start, _, filn, lnum, col, text = string.find(t, [[([^:]+):(%d+):(%d+):(.*)]])
...
--			else
--				return function(t)
--					local _, _, filn, lnum, col, text = string.find(t, [[([^:]+):(%d+):(%d+):(.*)]])
--					return filn, lnum, col, text
--				end
--			end
--		end)()
...
--	local fn = Module.Config.dailies
--		.. "/"
...
--	local defaults = Module.Config.calendar_opts
--	opts = opts or defaults
...
--	opts = opts or {}
--	local i = opts.i
--	opts.cwd = Module.Config.home
--	opts.tag_notation = Module.Config.tag_notation
...
--	if Module.Config.show_tags_theme == "get_cursor" then
--		opts = themes.get_cursor({
--			layout_config = {
--				height = math.min(math.floor(vim.o.lines * 0.8), #taglist),
--			},
--		})
--	elseif Module.Config.show_tags_theme == "ivy" then
--		opts = themes.get_ivy({
--			layout_config = {
--				prompt_position = "top",
--				height = math.min(math.floor(vim.o.lines * 0.8), #taglist),
...
--					display = string.format("%" .. max_tag_len .. "s ... (%3d matches)", entry.tag, #entry.details),
--					ordinal = entry.tag,
--				}
--			end,
--		}),
...
--		Module.Config.find_command = { "rg", "--files", "--sortr", "created" }
--	else
--		Module.Config.find_command = nil
--	end

--	-- this looks a little messy
--	if Module.Config.plug_into_calendar then
--		cfg.calendar_opts = cfg.calendar_opts or {}
--		Module.Config.calendar_opts = Module.Config.calendar_opts or {}
--		Module.Config.calendar_opts.weeknm = cfg.calendar_opts.weeknm or Module.Config.calendar_opts.weeknm or 1
--		Module.Config.calendar_opts.calendar_monday = cfg.calendar_opts.calendar_monday
--			or Module.Config.calendar_opts.calendar_monday
--			or 1
--		Module.Config.calendar_opts.calendar_mark = cfg.calendar_opts.calendar_mark
--			or Module.Config.calendar_opts.calendar_mark
--			or "left-fit"
--		SetupCalendar(Module.Config.calendar_opts)
--	end

--	-- setup extensions to filter for
--	Module.Config.filter_extensions = cfg.filter_extensions or { Module.Config.extension }

--	-- provide fake filenames for template loading to fail silently if template is configured off
--	Module.Config.template_new_note = Module.Config.template_new_note or "none"
--	Module.Config.template_new_daily = Module.Config.template_new_daily or "none"
--	Module.Config.template_new_weekly = Module.Config.template_new_weekly or "none"
...
--	if Module.Config.take_over_my_home == true then
--		if Module.Config.auto_set_filetype then
--			vim.cmd("au BufEnter " .. Module.Config.home .. "/*" .. Module.Config.extension .. " set ft=telekasten")
--		end
--	end
...
--		print("-----------------")
--		print(vim.inspect(Module.Config))
--	end

--	-- Convert all directories in full path
--	Module.Config.image_subdir = make_config_path_absolute(Module.Config.image_subdir)
--	Module.Config.dailies = make_config_path_absolute(Module.Config.dailies)
--	Module.Config.weeklies = make_config_path_absolute(Module.Config.weeklies)
--	Module.Config.templates = make_config_path_absolute(Module.Config.templates)
--end

--Module.find_notes = FindNotes
--Module.find_daily_notes = FindDailyNotes
--Module.search_notes = SearchNotes
--Module.insert_link = InsertLink
--Module.follow_link = FollowLink
--Module.setup = Setup
--Module.goto_today = GotoToday
--Module.new_note = CreateNote
--Module.goto_thisweek = GotoThisWeek
--Module.find_weekly_notes = FindWeeklyNotes
--Module.yank_notelink = YankLink
--Module.rename_note = RenameNote
--Module.new_templated_note = CreateNoteSelectTemplate
...
--Module.insert_img_link = InsertImgLink
--Module.preview_img = PreviewImg
--Module.browse_media = BrowseImg
--Module.taglinks = taglinks
--Module.show_tags = FindAllTags

---- Telekasten command, completion
--local TelekastenCmd = {
--	commands = function()
--		return {
--			{ "find notes", "find_notes", Module.find_notes },
--			{ "find daily notes", "find_daily_notes", Module.find_daily_notes },
--			{ "search in notes", "search_notes", Module.search_notes },
--			{ "insert link", "insert_link", Module.insert_link },
--			{ "follow link", "follow_link", Module.follow_link },
--			{ "goto today", "goto_today", Module.goto_today },
--			{ "new note", "new_note", Module.new_note },
--			{ "goto thisweek", "goto_thisweek", Module.goto_thisweek },
--			{ "find weekly notes", "find_weekly_notes", Module.find_weekly_notes },
--			{ "yank link to note", "yank_notelink", Module.yank_notelink },
--			{ "rename note", "rename_note", Module.rename_note },
--			{
--				"new templated note",
--				"new_templated_note",
...
--				"insert_img_link",
--				Module.insert_img_link,
--			},
--			{ "preview image under cursor", "preview_img", Module.preview_img },
--			{ "browse media", "browse_media", Module.browse_media },
--			{ "panel", "panel", Module.panel },
--			{ "show tags", "show_tags", Module.show_tags },
--		}
--	end,
--}
...
--		if Module.Config.command_palette_theme == "ivy" then
...
--Module.panel = TelekastenCmd.command
--Module.Command = TelekastenCmd
